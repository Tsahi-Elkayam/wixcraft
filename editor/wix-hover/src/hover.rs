//! Core hover logic.
//!
//! This module contains the [`HoverProvider`] which generates formatted
//! hover documentation for WiX elements, attributes, and values.
...
///
/// Takes loaded [`WixData`] and provides hover information for any position
/// in a WiX source file.
...
impl HoverProvider {
    ...
    ///
    /// # Arguments
    /// * `source` - The WiX XML source text
    /// * `line` - Line number (1-based)
    /// * `column` - Column number (1-based)
    ///
    /// # Returns
    /// `Some(HoverInfo)` with formatted documentation, or `None` if no hover
    /// information is available at the position.
...
mod tests {
    ...
    #[test]
    fn test_hover_more_standard_directories() {
        let (_temp, data) = create_test_data();
        let provider = HoverProvider::new(data);

        // Test various standard directories to cover all match arms
        let dirs = [
            ...
            ("System64Folder", "64-bit System"),
            ("WindowsFolder", "Windows directory"),
            ("TempFolder", "temporary"),
            ("LocalAppDataFolder", "local application data"),
            ("AppDataFolder", "roaming application data"),
            ("CommonAppDataFolder", "common application data"),
            ("DesktopFolder", "desktop"),
            ("StartMenuFolder", "Start Menu"),
            ("ProgramMenuFolder", "Programs folder"),
            ("StartupFolder", "Startup"),
            ("FontsFolder", "Fonts"),
            ("CustomFolder", "Standard Windows Installer"),
        ];

        for (dir_name, expected_text) in &dirs {
            let info = provider.hover_standard_directory(dir_name);
            assert!(
                info.contents
                    .to_lowercase()
    ...
    #[test]
    fn test_hover_more_builtin_properties() {
        let (_temp, data) = create_test_data();
        let provider = HoverProvider::new(data);

        // Test various builtin properties to cover all match arms
        let props = [
            ("ProductCode", "unique GUID"),
            ("ProductVersion", "version"),
            ("Manufacturer", "manufacturer"),
            ("UpgradeCode", "major upgrades"),
            ("ProductLanguage", "language ID"),
            ("INSTALLFOLDER", "installation directory"),
            ("INSTALLDIR", "installation directory"),
            ("CustomProp", "Built-in Windows Installer"),
        ];

        for (prop_name, expected_text) in &props {
            let info = provider.hover_builtin_property(prop_name);
            assert!(
                info.contents
                    .to_lowercase()
                    .contains(&expected_text.to_lowercase()),
