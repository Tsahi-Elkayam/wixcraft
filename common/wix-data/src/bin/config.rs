//! config - Manage WiX Data Layer configuration
//!
//! Usage:
...
use wix_data::config::{LintConfig, SourcesConfig};
use wix_data::Result;

#[derive(Parser)]
#[command(name = "config")]
#[command(about = "Manage WiX Data Layer configuration")]
#[command(version)]
struct Cli {
...
fn main() -> Result<()> {
    ...
    let config_path = cli
        .config
        .clone()
        .unwrap_or_else(|| PathBuf::from("config/sources.yaml"));
