//! harvest - Populate WiX Data Layer from sources
//!
//! Usage:
...
use wix_data::config::SourcesConfig;
use wix_data::db::Database;
use wix_data::harvest::Harvester;
use wix_data::{default_db_path, Result};

#[derive(Parser)]
#[command(name = "harvest")]
#[command(about = "Populate WiX Data Layer from sources")]
#[command(version)]
struct Cli {
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Validate => match SourcesConfig::load(&config_path) {
            Ok(config) => {
