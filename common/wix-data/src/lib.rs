...
pub const DEFAULT_DB_NAME: &str = "wix-data.db";

/// Errors that can occur in wix-data
#[derive(Error, Debug)]
pub enum WixDataError {
...
pub type Result<T> = std::result::Result<T, WixDataError>;

/// Main entry point for the WiX Data Layer
pub struct WixData {
    db: db::Database,
...
impl WixData {
    ...
    /// Open the default data store location
...
pub fn default_db_path() -> Result<PathBuf> {
    // Try in order:
    // 1. WIX_DATA_DATABASE env var
    // 2. ./database/wix-data.db (current working directory)
    // 3. database/wix-data.db relative to executable
    // 4. ~/.wixcraft/wix-data.db

    if let Ok(path) = std::env::var("WIX_DATA_DATABASE") {
    ...
    // Default to ~/.wixcraft/wix-data.db (for init)
    dirs::home_dir()
        .map(|h| h.join(".wixcraft").join(DEFAULT_DB_NAME))
        .ok_or_else(|| WixDataError::Config("Could not determine home directory".to_string()))
}
...
mod tests {
    ...
    fn test_default_db_name() {
        assert_eq!(DEFAULT_DB_NAME, "wix-data.db");
    }
}
