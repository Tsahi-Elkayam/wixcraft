//! Configuration management for WiX Knowledge Base
...
impl SourcesConfig {
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let config: SourcesConfig = serde_yaml::from_str(&content)
            .map_err(|e| WixKbError::Config(format!("Failed to parse YAML: {}", e)))?;
        Ok(config)
    }
    ...
    pub fn save<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        let content = serde_yaml::to_string(self)
            .map_err(|e| WixKbError::Config(format!("Failed to serialize YAML: {}", e)))?;
...
impl LintConfig {
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let config: LintConfig = serde_json::from_str(&content)
            .map_err(|e| WixKbError::Config(format!("Failed to parse JSON: {}", e)))?;
        Ok(config)
    }
