...
mod tests {
    ...
    fn test_deserialize_with_defaults() {
        // Test that serde default functions are called during deserialization
        let json = r#"{"sortElements": true}"#;
        let config: FormatConfig = serde_json::from_str(json).unwrap();

        // These should be set to their default values via serde's default functions
        assert_eq!(config.indent_size, 2); // default_indent_size()
        assert_eq!(config.max_line_width, 120); // default_max_line_width()
    ...
    #[test]
    fn test_find_wixfmtrc_without_json_extension() {
        let temp = TempDir::new().unwrap();
        let config_path = temp.path().join(".wixfmtrc");
        fs::write(&config_path, r#"{"indentSize": 3}"#).unwrap();
    ...
    #[test]
    fn test_find_wixfmt_json() {
        let temp = TempDir::new().unwrap();
        let config_path = temp.path().join("wixfmt.json");
        fs::write(&config_path, r#"{"indentSize": 5}"#).unwrap();
    ...
    fn test_config_error_parse_display() {
        let err = ConfigError::Parse {
            path: "/test/config.json".into(),
            source: serde_json::from_str::<FormatConfig>("invalid").unwrap_err(),
        };
        let msg = err.to_string();
        assert!(msg.contains("Failed to parse config file"));
        assert!(msg.contains("/test/config.json"));
    }
}
