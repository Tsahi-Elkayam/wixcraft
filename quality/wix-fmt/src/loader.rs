...
mod tests {
    ...
    fn test_element_no_attributes() {
        ...
        fs::write(elements_dir.join("fragment.json"), element).unwrap();

        let data = WixData::load(temp.path()).unwrap();

        // Should not have attr_priority entry for Fragment
        assert!(data.get_attr_priority("Fragment").is_none());
        // But should have child_order
        assert!(data.get_child_order("Fragment").is_some());
    }

    #[test]
    fn test_attr_priority_id_in_required() {
        let temp = TempDir::new().unwrap();
        let elements_dir = temp.path().join("elements");
        fs::create_dir(&elements_dir).unwrap();

        // Element where Id is required
        let element = r#"{
            "name": "Directory",
            "children": [],
            "attributes": {
        ...
        fs::write(elements_dir.join("directory.json"), element).unwrap();

        let data = WixData::load(temp.path()).unwrap();
        let priority = data.get_attr_priority("Directory").unwrap();

        // Id should be first among required
        assert_eq!(priority[0], "Id");
        // Name should be second (required, but after Id)
        assert_eq!(priority[1], "Name");
        // Zebra is optional, should be last
        assert_eq!(priority[2], "Zebra");
    }

    #[test]
    fn test_attr_priority_id_in_optional() {
        let temp = TempDir::new().unwrap();
        let elements_dir = temp.path().join("elements");
        fs::create_dir(&elements_dir).unwrap();

        // Element where Id is optional alongside other optional attrs
        let element = r#"{
            "name": "File",
            "children": [],
            "attributes": {
        ...
        fs::write(elements_dir.join("file.json"), element).unwrap();

        let data = WixData::load(temp.path()).unwrap();
        let priority = data.get_attr_priority("File").unwrap();

        // Source is required, comes first
        assert_eq!(priority[0], "Source");
        // Id should be first among optional (before Name)
        assert_eq!(priority[1], "Id");
        assert_eq!(priority[2], "Name");
    }

    #[test]
    fn test_element_no_children() {
        let temp = TempDir::new().unwrap();
        let elements_dir = temp.path().join("elements");
        fs::create_dir(&elements_dir).unwrap();
        ...
        // Should not have child_order for File
        assert!(data.get_child_order("File").is_none());
    }

    #[test]
    fn test_no_elements_dir() {
        let temp = TempDir::new().unwrap();
        // Don't create elements directory
    ...
    fn test_load_error_parse_display() {
        ...
        let elements_dir = temp.path().join("elements");
        fs::create_dir(&elements_dir).unwrap();
        fs::write(elements_dir.join("invalid.json"), "{invalid}").unwrap();
